...
struct Message {
    ...
    constexpr Message(char *_str, size_t _length) : str(_str), length(_length) {}
    char *str;
    size_t length;
};
...
void UARTE0_UART0_IRQHandler() {
    ...
    if (ptrBuffer[tail].str[ptrBuffer[tail].length])
        ptrBuffer[tail].str[ptrBuffer[tail].length] = SERIAL_RELEASED;
